...
export const FeaturedProducts = () => {
  return (
    <section className="flex flex-col items-center justify-around px-3 dark:text-gray-100 text-gray-900 gap-5 mt-20 bg-white pt-[50px] dark:pt-[20px] dark:bg-neutral-950">
      ...
      <div className="featuredProductsItems grid grid-cols-2 gap-2.5 gap-y-[50px] md:gap-y-[100px] md:grid-cols-3 lg:grid-cols-4">
        {
          ItemsData.map((item) => {
            const {img, price, name, discount, id} = item;
              return (
                <ItemComponent key={id} img = {img} price={price} name={name} discount={discount} />
              )
          })
        }
      </div>
